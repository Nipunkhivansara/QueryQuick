...
import { Box, TextField, IconButton, Menu, MenuItem, Divider, Typography, Button } from '@mui/material';
...
const Cell = ({ type, value, onChange, onDelete, handleMenuOpen }) => {
    const inputProps = { disableUnderline: true, style: { ...styles[type], paddingLeft: '0px' } };
...
const Notebook = ({ menuBarWidth, open, logout, user, handleDrawerToggle }) => {
    ...
    const addCell = (type) => {
        setCells([...cells, { cellId: nextCellID.current, type, value: '' , database: '', databaseType: '', prompt: '', query: ''}]);
        nextCellID.current++;
        handleMenuClose();
    };
    ...
    const onQueryEngineChange = (index, prop, val) => {
        const newCells = [...cells];
        if(prop === 'databaseType') {setDatabaseType(val); newCells[index].databaseType = val;}
        if(prop === 'database') {setDatabase(val); newCells[index].database = val;}
        if(prop === 'prompt') {setPrompt(val); newCells[index].prompt = val;}
        if(prop === 'query') {setQuery(val); newCells[index].query = val;}
        setCells(newCells);
    }
            ...
            <Box sx={{ zIndex: 1, display: 'flex', minHeight: '100%', minWidth: '100%', bgcolor: '#222B3D' }}>
                ...
                <Box sx={{
                    ...
                    <Box sx={{ width: '100%', paddingLeft: '8px' }}>
                        <TextField
                            ...
                            InputProps={{
                                ...
                                style: { color: '#fff', fontSize: '2.125rem', paddingLeft: '0px' },
                            }}
                            sx={{
                                '& .MuiInputBase-input': {
                                    color: '#fff',
                        ...
                        />
                        <Typography variant="body1" color="white" sx={{ marginTop: '4px', paddingLeft: '0px' }}>
                    ...
                    >
                        ...
                        <MenuItem onClick={() => addCell('queryEngine')}>Query Engine</MenuItem>
                        ...
                        <MenuItem onClick={() => addCell('horizontalDivider')}>Horizontal Divider</MenuItem>
                    </Menu>
                </Box>
            </Box>
        </div>
